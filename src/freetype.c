	...
	for ( x = start_x, buff_x = 0; x < end_x; buff_x++, x++ )	//y表示起点y，x表示起点x
	{
		for ( y = start_y, buff_y = 0; y < end_y; buff_y++, y++ )	//y表示起点y，x表示起点x
		{
			...
			if(bitmap->buffer[buff_y * bitmap->width + buff_x] )	//判断该位上是不是为1，1则表明需要描点
				//display_screen_point(start_x+buff_x , start_y+buff_y ,  color, lcd_buf_ptr);		//在当前x位置加上p的偏移量（p表示buff中列的移动）
				*(lcd_buf_ptr+800*(start_y+buff_y)+start_x+buff_x) = color;
...
int display_font_to_screen(char *string, int size, unsigned int color, unsigned int start_x, unsigned int start_y, unsigned int *lcd_fb_addr)
{
	...
	int ret = mbstowcs(dest, string, strlen(string)+1);
	if (ret <= 0) 
	{
		perror("to error:");
		return -1;
	...
	//显示汉字
	Lcd_Show_FreeType(dest, size, color, start_x, start_y, lcd_fb_addr);
